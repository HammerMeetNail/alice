...
- `cmd/edge-agent/`: per-user edge runtime with five operating modes: default poll-and-publish, `-dry-run` (preview what would be published without contacting the server), `-bootstrap-connector` (OAuth PKCE), `-register-watches` (provider-side push channel setup), and `-serve-webhooks` (webhook intake server); `-validate-config` loads, validates, and prints the normalized config then exits; `-generate-config` prints a complete starter config with all fields to stdout and a field guide to stderr
