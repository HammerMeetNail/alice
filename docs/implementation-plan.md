...
- the public surface now includes HTTP plus a local stdio MCP server for the implemented Reporter and Gatekeeper tools, while the edge runtime now also exposes a local GitHub webhook endpoint for push-based connector input
...
Build on the current paginated, replacement-aware edge runtime with richer push paths and derivation.
